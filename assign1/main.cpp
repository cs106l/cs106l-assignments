...
void parse_csv(std::string filename, std::vector<Course> &courses) {
  ...
  std::string redunency ;
   std::ifstream ifs(filename);
  std::getline(ifs,redunency);
  std::string elem;

  while(std::getline(ifs,elem)){
    std::vector<std::string>cour;
    cour = split(elem,',');
  Course item{cour[0],cour[1],cour[2]};
...
void write_courses_offered(std::vector<Course> &all_courses) {
  /* (STUDENT TODO) Your code goes here... */
    ...
    std::vector<Course>m;
    for(auto item : all_courses){
       if(item.quarter != "null"){
         m.push_back(item);
         ofs<<item.title<<","<<item.number_of_units<<","<<item.quarter<<'\n';
       }
    }
    for(auto elem : m){
...
void write_courses_not_offered(std::vector<Course> unlisted_courses) {
  ...
  ofs<<"Title,"<<"Number of Units,"<<"Quarter"<<'\n';
  for(auto item : unlisted_courses){
      ofs<<item.title<<","<<item.number_of_units<<","<<item.quarter<<'\n';
    }
  }
